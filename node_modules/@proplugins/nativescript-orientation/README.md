...
## Installation 
NativeScript 7.0, 8.0 and later
```
tns plugin add @proplugins/nativescript-orientation@latest
```


NativeScript 6.2 and later
```
tns plugin add @proplugins/nativescript-orientation@2.4.2
```

NativeScript 6.1 and before
```
tns plugin add @proplugins/nativescript-orientation@2.3.2
```
...
```js
require( "@proplugins/nativescript-orientation" );
```
...
**PLEASE NOTE both *.ns-landscape* and *.ns-portrait* are now built into the NS 6.x -- use them over the *.landscape* as in a future version I will be removing this code**     

This plugin's main use is to manually control the orientation; it still supports the above .landscape ability; but that ability will be removed in a later version as the .ns-landscape value is now built into the framework and should be used instead.
...
#### orientation.getOrientation(androidSensors?|iosForcedValue?)
##### optional androidSensors === true, will return you sensor values on android verses screen size calculation 
##### options iOSForcedValue === true, will return you the forced value if you have locked the screen rotation, otherwise this function returns the actual physical layout.
...
#### orientation.getRotationLocked() - Returns true if you can't auto-rotate
Use `orientation.enableRotaion()` to re-enable automatic orientation.
*  New in 2.5.0 (NS 7 only)
